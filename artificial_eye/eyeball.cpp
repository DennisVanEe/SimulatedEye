...
syn_eye::EyeBall::EyeBall(glm::vec3 pos, float radius, float retinaWidth, float retinaHeight, ImagePlane* image, World* world) :
	m_transform(),
	m_world(world),
	m_imageBuffer(image),
	m_model(EYE_MODEL_DIR)
{
	...
	radius *= 0.5; // this is just a hack, so, yeah
...
void syn_eye::EyeBall::calculateImage()
{
...
void syn_eye::EyeBall::draw(Renderer* renderer)
{
...
syn_eye::Color syn_eye::EyeBall::rayTrace(Ray ray)
{
	...
	for (const RaySphere& s : sphereList)
	{
		if (s.intersect(ray, 0).first > 0)
			return Color(255, 255, 255); // white for now
	}
	return Color(0, 0, 0); // Color(0, 0, 0); // black for now
}