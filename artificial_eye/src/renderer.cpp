...
ee::Renderer& ee::Renderer::startRenderer(size_t width, size_t height)
{
...
ee::Renderer::Renderer(size_t width, size_t height) :
...
bool ee::Renderer::keepRendering() const
{
	return !glfwWindowShouldClose(m_window);
}

void ee::Renderer::processInput(float deltaTime)
{
...
void ee::Renderer::mouseCallback(GLFWwindow* window, double xpos, double ypos)
{
	if (m_firstMouse)
	{
		m_lastX = xpos;
...
void ee::Renderer::scrollCallback(GLFWwindow* window, double xoffset, double yoffset)
{
	m_camera.processSInput(yoffset);
}

void ee::Renderer::draw(Drawable& item)
{
